...
static void server(int sfd) {
  while (1) {
    struct timeval timeout={CCCD_REAP};	// seconds between reaps
    fd_set fds;
    FD_ZERO(&fds);
    FD_SET(sfd,&fds);
    ...
    while (waitpid(0,0,WNOHANG)>0); // reap exited children
  }
}
...
static void open_stderr() {
  int fd=dup(2);
  stderr=freopen(CCCD_ERRFILE,"w",stderr);
  if (!stderr) {
    stderr=fdopen(fd,"w");
